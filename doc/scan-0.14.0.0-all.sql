...
CREATE TABLE `address` (
  ...
  `type` int NOT NULL COMMENT '地址类型 :1账号,2内置合约 ,3EVM合约,4WASM合约',
  ...
  `token_qty` int NOT NULL DEFAULT '0' COMMENT 'erc20 token对应的交易数',
...
CREATE TABLE `delegation` (
  ...
  `is_history` int NOT NULL DEFAULT '2' COMMENT '是否为历史:\r\n1是,\r\n2否',
...
CREATE TABLE `erc20_token` (
  ...
  `status` int DEFAULT '0' COMMENT '合约状态 1 可见，0 隐藏',
  `block_timestamp` datetime DEFAULT NULL COMMENT '上链时间',
  `tx_count` int DEFAULT NULL COMMENT '合约内交易数',
  `holder` int NOT NULL DEFAULT '0' COMMENT 'erc20 token对应的持有人',
...
/*Table structure for table `erc20_token_address_rel` */
...
CREATE TABLE `erc20_token_address_rel` (
  ...
  `symbol` varchar(32) COLLATE utf8mb4_unicode_ci DEFAULT NULL COMMENT '合约符号',
  `decimal` int DEFAULT NULL COMMENT '合约精度',
  `tx_count` int NOT NULL DEFAULT '0' COMMENT '地址token交易数',
...
/*Table structure for table `erc20_token_detail` */
...
/*Table structure for table `erc20_token_transfer_record` */
...
CREATE TABLE `erc20_token_transfer_record` (
  ...
  `decimal` int DEFAULT NULL COMMENT '代币精度',
  ...
  `result` int DEFAULT '1' COMMENT '转账结果 1成功，0失败',
...
CREATE TABLE `gas_estimate` (
  ...
  `sbn` bigint NOT NULL COMMENT '最新的质押交易块高',
  `epoch` bigint NOT NULL DEFAULT '0' COMMENT '委托未计算周期',
...
CREATE TABLE `network_stat` (
  ...
  `address_qty` int unsigned NOT NULL DEFAULT '0' COMMENT '地址数',
...
CREATE TABLE `node` (
  ...
  `is_recommend` int NOT NULL DEFAULT '2' COMMENT '官方推荐 :1是\r\n2:否',
  ...
  `status` int NOT NULL DEFAULT '1' COMMENT '节点状态:1候选中,2退出中,3已退出,4已锁定',
  `is_consensus` int NOT NULL DEFAULT '2' COMMENT '是否共识周期验证人:1是,2否',
  `is_settle` int NOT NULL DEFAULT '2' COMMENT '是否结算周期验证人:1是,2否',
  `is_init` int NOT NULL DEFAULT '2' COMMENT '是否为链初始化时内置的候选人:1是,2否',
  ...
  `exception_status` int NOT NULL DEFAULT '1' COMMENT '1正常,2低出块异常,3被双签,4因低出块被惩罚(例如在连续两个周期当选验证人，但在第一个周期出块率低),5因双签被惩罚',
...
CREATE TABLE `proposal` (
  ...
  `yeas` bigint NOT NULL DEFAULT '0' COMMENT '赞成票',
  `nays` bigint NOT NULL DEFAULT '0' COMMENT '反对票',
  `abstentions` bigint NOT NULL DEFAULT '0' COMMENT '弃权票',
  `accu_verifiers` bigint NOT NULL DEFAULT '0' COMMENT '在整个投票期内有投票资格的验证人总数',
  `status` int NOT NULL DEFAULT '1' COMMENT '提案状态:1投票中,2通过,3失败,4预升级,5生效,6被取消',
  ...
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  `completion_flag` int NOT NULL DEFAULT '2' COMMENT '提案相关数据是否补充完成标识:1是,2否',
...
CREATE TABLE `slash` (
  ...
  `is_quit` int NOT NULL DEFAULT '1' COMMENT '是否退出:1是,2否',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`hash`),
  KEY `node_id` (`node_id`) USING BTREE
...
CREATE TABLE `staking` (
  ...
  `cur_cons_block_qty` bigint NOT NULL DEFAULT '0' COMMENT '当前共识周期出块数',
  `pre_cons_block_qty` bigint NOT NULL DEFAULT '0' COMMENT '上个共识周期出块数',
  `annualized_rate_info` longtext COLLATE utf8mb4_unicode_ci COMMENT '最近几个结算周期收益和质押信息',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `tx_bak` (
  ...
  `num` bigint NOT NULL COMMENT '区块号',
