...
CREATE TABLE `n_opt_bak` (
  ...
  `node_id` varchar(130) NOT NULL COMMENT '节点id',
  `type` int(2) NOT NULL COMMENT '操作类型:1创建,2修改,3退出,4提案,5投票,6双签,7出块率低,11解除锁定',
...
CREATE TABLE `network_stat` (
  ...
  `issue_rates` varchar(2048) DEFAULT NULL COMMENT '增发比例',
  `create_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
...
CREATE TABLE `node` (
  ...
  `node_name` varchar(128) NOT NULL DEFAULT '' COMMENT '节点名称(质押节点名称)',
  ...
  `status` int(2) NOT NULL DEFAULT '1' COMMENT '节点状态:1候选中,2退出中,3已退出,4已锁定',
...
/*Table structure for table `sta_dele` */
...
/*Table structure for table `sta_file` */
...
/*Table structure for table `sta_guess` */
...
/*Table structure for table `sta_guess_list` */
...
/*Table structure for table `sta_incite_block` */
...
/*Table structure for table `sta_incite_reward` */
...
/*Table structure for table `sta_reward` */
...
/*Table structure for table `sta_switch` */
