...
public class DelegateAdjustContext extends AbstractAdjustContext {
    ...
    void calculateAmountAndStatus() {
        if(
            ...
            adjustParam.setDelegateReleased(
                adjustParam.getDelegateReleased()
                .subtract(adjustParam.getHes())
                .subtract(adjustParam.getLock())
            );
            // 2、从节点的statDelegateReleased减掉lock
            adjustParam.setNodeStatDelegateReleased(
                adjustParam.getNodeStatDelegateReleased()
                .subtract(adjustParam.getHes())
                .subtract(adjustParam.getLock())
            );
            // 3、从质押的statDelegateReleased减掉hes和lock
            adjustParam.setStakeStatDelegateReleased(
                adjustParam.getStakeStatDelegateReleased()
                .subtract(adjustParam.getHes())
                .subtract(adjustParam.getLock())
            );
        }else{
            ...
            adjustParam.setNodeStatDelegateValue(
                adjustParam.getNodeStatDelegateValue()
                .subtract(adjustParam.getHes())
                .subtract(adjustParam.getLock())
            );
            // 3、从质押的statDelegateHes减掉hes
            adjustParam.setStakeStatDelegateHes(
                adjustParam.getStakeStatDelegateHes().subtract(adjustParam.getHes())
            );
            // 4、从质押的statDelegateLocked减掉lock
            adjustParam.setStakeStatDelegateLocked(
                adjustParam.getStakeStatDelegateLocked().subtract(adjustParam.getLock())
            );
