...
CREATE TABLE `tx_bak` (
                          ...
                          `chain_Id` bigint(20) COMMENT '链id',
                          `raw_eth_tx_type` int(4) COMMENT '0 - 传统交易（旧） 1 - AccessList交易 2 - DynamicFee交易',
                          `max_fee_per_gas` varchar(255) COMMENT '10进制数字当字符串存储',
                          `max_priority_fee_per_gas` varchar(255) COMMENT '10进制数字当字符串存储',
                          `access_list_info` longtext COMMENT 'access_list的json序列化存储',
                           PRIMARY KEY (`id`),
                          KEY `idx_time` (`time`)
) COMMENT='交易备份表';
...
-- scan-v1.5.0.0-to-v1.5.0.1-false.sql 新增的SQL
