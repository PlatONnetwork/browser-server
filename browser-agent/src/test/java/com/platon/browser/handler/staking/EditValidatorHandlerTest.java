...
public class EditValidatorHandlerTest extends TestBase {
    ...
    public void testHandler () {
        ...
        transactions.forEach(transactionBean -> {
            if (transactionBean.getTxType().equals(CustomTransaction.TxTypeEnum.EDIT_VALIDATOR.code)) {
                try {
                    editValidatorHandler.handle(eventContext);
                    assertTrue(true);
                    logger.info("[EditValidatorHandlerTest] Test Pass");
                } catch (Exception e) {
                    assertTrue(false);
                    logger.info("[EditValidatorHandlerTest] Test Fail");
                }
            }
        });
    }
