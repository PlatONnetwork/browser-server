...
import com.platon.browser.handler.staking.CreateValidatorHandlerTest;
...
public class NetworkStatStatisticHandlerTest extends TestBase {
    ...
    public void testHandler () {
        ...
        transactions.forEach(transactionBean -> {
            try {
                networkStatStatisticHandler.handle(eventContext);
                assertTrue(true);
                logger.info("[NetworkStatStatisticHandlerTest] Test Pass");
            } catch (Exception e) {
                assertTrue(false);
                logger.info("[NetworkStatStatisticHandlerTest] Test Fail");
            }
        });
    }
}