...
import com.platon.browser.service.TransactionServiceTest;
    ...
    */
    ...
    public void testHandler () {
        ...
        transactions.forEach(transactionBean -> {
            if (transactionBean.getTxType().equals(CustomTransaction.TxTypeEnum.DELEGATE.code)) {
                try {
                    delegateHandler.handle(eventContext);
                    assertTrue(true);
                    logger.info("[DelegateHandlerTest] Test Pass");
                } catch (Exception e) {
                    assertTrue(false);
                    logger.info("[DelegateHandlerTest] Test Fail");
                }
            }
        });
    }
