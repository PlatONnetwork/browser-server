package com.platon.browser.handler.slash;
...
public class ReportValidatorHandlerTest extends TestBase {
    ...
    public void testHandler () {
        ...
        transactions.forEach(transactionBean -> {
            if (transactionBean.getTxType().equals(CustomTransaction.TxTypeEnum.REPORT_VALIDATOR.code)) {
                try {
                    reportValidatorHandler.handle(eventContext);
                    assertTrue(true);
                    logger.info("[ReportValidatorHandlerTest] Test Pass");
                } catch (Exception e) {
                    assertTrue(false);
                    logger.info("[ReportValidatorHandlerTest] Test Fail");
                }
            }
        });
    }
