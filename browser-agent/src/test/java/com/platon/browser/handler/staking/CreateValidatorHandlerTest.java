...
import com.platon.browser.handler.slash.ReportValidatorHandlerTest;
...
public class CreateValidatorHandlerTest extends TestBase {
    ...
    public void testHandler () {
        ...
        transactions.forEach(transactionBean -> {
            if (transactionBean.getTxType().equals(CustomTransaction.TxTypeEnum.CREATE_VALIDATOR.code)) {
                try {
                    createValidatorHandler.handle(eventContext);
                    assertTrue(true);
                    logger.info("[CreateValidatorHandlerTest] Test Pass");
                } catch (Exception e) {
                    assertTrue(false);
                    logger.info("[CreateValidatorHandlerTest] Test Fail");
                }
            }
        });
    }
