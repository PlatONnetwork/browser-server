...
public class IncreaseStakingHandlerTest  extends TestBase {
    ...
    public void testHandler () {
        ...
        transactions.forEach(transactionBean -> {
            if (transactionBean.getTxType().equals(CustomTransaction.TxTypeEnum.INCREASE_STAKING.code)) {
                try {
                    increaseStakingHandler.handle(eventContext);
                    assertTrue(true);
                    logger.info("[IncreaseStakingHandlerTest] Test Pass");
                } catch (Exception e) {
                    assertTrue(false);
                    logger.info("[IncreaseStakingHandlerTest] Test Fail");
                }
            }
        });
    }
