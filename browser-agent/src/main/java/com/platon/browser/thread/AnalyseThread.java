...
//    }
//
//    public void analyse(List<PlatonBlock> blocks){
...
//                    latch.countDown();
//                }
//                if((System.currentTimeMillis()-threadTime)>0) logger.debug("Analyse Thread ID={},NAME={} : -> {}ms",Thread.currentThread().getId(),Thread.currentThread().getName(),System.currentTimeMillis()-threadTime);
//            })
//        );
//
//        try {
...
//        detectMissingBlocks();
//
//        if((System.currentTimeMillis()-startTime)>0) logger.debug("databaseService.flush(result): -> {}ms",System.currentTimeMillis()-startTime);
//    }
//
//    /**
//     * 检测缺失块
//     */
//    private void detectMissingBlocks(){
...
//            List<BlockMissing> missingBlocks = blockMissingMapper.selectByExample(example);
//            if(missingBlocks.size()==0) return;
//
//            List<PlatonBlock> concurrentBlocks = new ArrayList<>();
//            missingBlocks.forEach(missing->{
//                try {
//                    PlatonBlock ethBlock = platon.getWeb3j(chainId).platonGetBlockByNumber(DefaultBlockParameter.valueOf(BigInteger.valueOf(missing.getNumber())),true).send();
