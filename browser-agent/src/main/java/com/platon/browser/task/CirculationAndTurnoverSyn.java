...
public class CirculationAndTurnoverSyn {
    private static Logger logger = LoggerFactory.getLogger(CirculationAndTurnoverSyn.class);
    @Autowired
    private BlockChain blockChain;
    @Autowired
    private PlatonClient platonClient;
    private static final String stakingContractAdd = "0x1000000000000000000000000000000000000002";
    private static final String restrictingContractAdd = "0x1000000000000000000000000000000000000001";
    ...
    protected void syn () {
        try {
            ...
            //rpc获取锁仓余额
            BigInteger lockContractBalance = platonClient.getWeb3j().platonGetBalance(restrictingContractAdd,
                    DefaultBlockParameter.valueOf(BigInteger.valueOf(blockChain.getCurBlock().getBlockNumber().longValue()))).send().getBalance();
            //rpc获取质押余额
            BigInteger stakingContractBalance = platonClient.getWeb3j().platonGetBalance(stakingContractAdd,
                    DefaultBlockParameter.valueOf(BigInteger.valueOf(blockChain.getCurBlock().getBlockNumber().longValue()))).send().getBalance();
