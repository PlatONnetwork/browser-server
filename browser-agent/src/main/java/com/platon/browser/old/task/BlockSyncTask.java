...
import com.platon.browser.old.engine.cache.AddressCacheUpdater;
import com.platon.browser.old.engine.cache.CacheHolder;
import com.platon.browser.old.engine.cache.NodeCache;
import com.platon.browser.old.engine.cache.StakingCacheUpdater;
...
import com.platon.browser.old.service.*;
...
public class BlockSyncTask extends BaseTask{
     ...
     * 初始化已有业务数据
     */
    public void init () throws BlockNumberException, CandidateException, com.platon.browser.old.exception.BusinessException, CacheConstructException, InterruptedException {
    ...
    public Boolean collect() throws Exception {
        ...
        if (!blockNumbers.isEmpty()) {
            ...
            if (!blocks.isEmpty()){
                ...
                } catch (com.platon.browser.old.exception.BusinessException e) {
    ...
    private void batchSave(List<CustomBlock> basicData, BlockChainStage bizData) throws com.platon.browser.old.exception.BusinessException {
        try{
            // 入库前更新统计信息
